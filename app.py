...
# Vamos verificar a variável do Groq
groq_key_env = os.getenv("GROQ_API_KEY")
if groq_key_env:
    print("DIAGNÓSTICO: Variável GROQ_API_KEY foi ENCONTRADA.")
else:
    print("DIAGNÓSTICO: ERRO! Variável GROQ_API_KEY NÃO FOI ENCONTRADA (valor está Nulo).")

# Vamos verificar uma variável de teste
test_key_env = os.getenv("TEST_KEY")
if test_key_env:
    print(f"DIAGNÓSTICO: Variável TEST_KEY foi ENCONTRADA. Valor: {test_key_env}")
else:
    print("DIAGNÓSTICO: Variável TEST_KEY NÃO FOI ENCONTRADA.")
...
try:
    # O 'client' agora é inicializado usando a variável que lemos lá em cima
    client = Groq(api_key=groq_key_env)
    if not groq_key_env:
        raise ValueError("GROQ_API_KEY não encontrada no ambiente.")
    print("Cliente Groq inicializado com sucesso.") # Esta linha só vai rodar se a chave foi encontrada
except Exception as e:
    print(f"Erro fatal ao inicializar o cliente Groq: {e}")
    client = None

# ... (O RESTO DO ARQUIVO É IDÊNTICO, NÃO PRECISA COPIAR DAQUI PARA BAIXO, SÓ GARANTA QUE O SEU ESTEJA IGUAL) ...
# ... (MAS PARA GARANTIR, VOU COLAR O RESTO DO CÓDIGO ABAIXO) ...
