...
try:
    ...
    if not groq_api_key:
        raise ValueError("GROQ_API_KEY não encontrada no arquivo .env ou no ambiente.")
    
    client = Groq(api_key=groq_api_key)
    print("Cliente Groq inicializado com sucesso.") # Log útil para o deploy
...
def analisar_email_com_ia(texto_email: str) -> dict:
    ...
    """
    if not client:
        # Este erro será mostrado no frontend se a chave falhar
...
def processar_email():
    ...
    try:
        ...
        print(f"Erro no endpoint /processar_email: {e}") # Log de erro no servidor
...
def upload_arquivo():
    ...
    try:
        ...
        print(f"Erro no endpoint /upload_arquivo: {e}") # Log de erro no servidor
        return jsonify({"error": f"Erro interno ao processar arquivo: {e}"}), 500


# --- Inicialização do Servidor ---
if __name__ == '__main__':
    # 'debug=True' é apenas para desenvolvimento local.
    # Gunicorn irá rodar o objeto 'app' em produção.
